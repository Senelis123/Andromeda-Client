...
pub fn atomic_write(path: &Path, bytes: &[u8]) -> Result<(), AppError> {
    let parent = path.parent().ok_or_else(|| AppError::FileSystem {
        operation: "locate parent directory for", path: path.to_path_buf(),
    ...
    let mut file = fs::File::create(&temporary).map_err(|source| fs_error("create", &temporary, source))?;
    file.write_all(bytes).map_err(|source| fs_error("write", &temporary, source))?;
    file.sync_all().map_err(|source| fs_error("flush", &temporary, source))?;
    if path.exists() {
...
fn fs_error(operation: &'static str, path: &Path, source: std::io::Error) -> AppError {
    AppError::FileSystem { operation, path: path.to_path_buf(), source }
}
