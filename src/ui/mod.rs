...
use iced::widget::{button, column, container, horizontal_rule, progress_bar, row, scrollable, text, Space};

use crate::{app::{AppState, Page}, config::{SettingsRepository, ThemePreference}, domain::TaskState, platform::AppPaths};
...
#[derive(Debug, Clone)]
enum Message {
    Navigate(Page), StartDemo, CancelDemo, Tick, DismissNotice,
    SetTheme(ThemePreference), ToggleSnapshots,
}
...
pub fn run(paths: AppPaths) -> iced::Result {
    ...
    let outcome = repository.load().unwrap_or_else(|error| {
        ...
        crate::config::LoadOutcome { settings: Default::default(), recovery_notice: Some(format!("Settings could not be loaded: {error}")) }
    });
    let app = Launcher { state: AppState::new(outcome.settings, outcome.recovery_notice), settings_repository: repository };
    iced::application(Launcher::title, Launcher::update, Launcher::view)
        .subscription(Launcher::subscription)
        .theme(Launcher::theme)
        .window(iced::window::Settings { size: iced::Size::new(1100.0, 720.0), min_size: Some(iced::Size::new(900.0, 600.0)), ..Default::default() })
        .run_with(|| (app, iced::Task::none()))
}

impl Launcher {
    fn title(&self) -> String { format!("Andromeda Client — {}", page_name(self.state.page)) }
    fn theme(&self) -> Theme { match self.state.settings.theme { ThemePreference::Light => Theme::Light, ThemePreference::Dark => Theme::Dark, ThemePreference::System => Theme::TokyoNight } }
    fn subscription(&self) -> Subscription<Message> {
        if self.state.task.as_ref().is_some_and(|task| task.state == TaskState::Running) {
    ...
    fn update(&mut self, message: Message) {
        match message {
            ...
            Message::StartDemo => { self.state.start_demo(); self.state.page = Page::Downloads; }
            ...
            Message::SetTheme(theme) => { self.state.settings.theme = theme; self.persist_settings(); }
            Message::ToggleSnapshots => { self.state.settings.show_snapshots = !self.state.settings.show_snapshots; self.persist_settings(); }
        }
    }
    ...
    fn view(&self) -> Element<'_, Message> {
        let nav = column![text("ANDROMEDA").size(24)]
            .spacing(10).padding(18).width(Length::Fixed(190.0))
        ...
        let mut content = column![page_header(page_name(self.state.page)), self.page_content()].spacing(18).padding(28);
        if let Some(notice) = &self.state.recovery_notice {
            content = content.push(container(row![text(notice).width(Length::Fill), button("Dismiss").on_press(Message::DismissNotice)].spacing(12)).padding(12));
        }
        row![container(nav).height(Length::Fill), scrollable(content).width(Length::Fill).height(Length::Fill)].into()
    }
    ...
    fn downloads_page(&self) -> Element<'_, Message> {
        if let Some(task) = &self.state.task {
            let action = if task.state == TaskState::Running { button("Cancel").on_press(Message::CancelDemo) } else { button("Run again").on_press(Message::StartDemo) };
            column![text("Demonstration task").size(22), text(&task.phase), progress_bar(0.0..=1.0, task.percentage()), text(format!("{}% complete — {:?}", (task.percentage() * 100.0) as u8, task.state)), action].spacing(14).into()
        } else {
    ...
    fn settings_page(&self) -> Element<'_, Message> {
        let snapshots = if self.state.settings.show_snapshots { "Snapshots: shown" } else { "Snapshots: hidden" };
        column![
...
fn nav_button(label: &'static str, page: Page, selected: Page) -> Element<'static, Message> {
    let label = if page == selected { format!("› {label}") } else { label.into() };
    button(text(label).width(Length::Fill)).on_press(Message::Navigate(page)).width(Length::Fill).into()
}
fn page_header(name: &str) -> Element<'_, Message> { row![text(name).size(32), Space::new().width(Length::Fill), text("Milestone 1")].align_y(iced::Alignment::Center).into() }
fn placeholder<'a>(title: &'a str, body: &'a str) -> Element<'a, Message> { container(column![text(title).size(22), text(body)].spacing(10)).padding(22).into() }
fn page_name(page: Page) -> &'static str { match page { Page::Home => "Home", Page::Instances => "Instances", Page::Downloads => "Downloads", Page::Accounts => "Accounts", Page::Logs => "Logs", Page::Settings => "Settings" } }
